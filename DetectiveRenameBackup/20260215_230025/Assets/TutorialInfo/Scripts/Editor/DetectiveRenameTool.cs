...
public class DetectiveRenameTool : EditorWindow
{
    ...
    private static readonly (string pattern, string replacement, bool isRegex)[] rules = new[]
    {
        ...
        ("namespace Eduzo.Games.CarToll.UI", "namespace Eduzo.Games.DetectiveGame.UI", false),
        ("namespace Eduzo.Games.CarToll.Data", "namespace Eduzo.Games.DetectiveGame.Data", false),
        ("namespace Eduzo.Games.CarToll", "namespace Eduzo.Games.DetectiveGame", true), // regex to allow word boundary variants
        ("using Eduzo.Games.CarToll.UI", "using Eduzo.Games.DetectiveGame.UI", false),
        ("using Eduzo.Games.CarToll.Data", "using Eduzo.Games.DetectiveGame.Data", false),
    ...
    private void OnGUI()
    {
        EditorGUILayout.HelpBox("Replaces 'CarToll' → 'DetectiveGame' across scripts. Use Preview before Apply. Renaming files will use AssetDatabase.MoveAsset to preserve .meta GUIDs.", MessageType.Info);
        ...
        renameFiles = EditorGUILayout.Toggle("Rename Files (CarToll -> DetectiveGame)", renameFiles);
    ...
    private void ScanAndPreview()
    {
        ...
        foreach (var file in csFiles)
        {
            try
            {
                ...
                bool willRenameFile = renameFiles && fileName.Contains("CarToll", StringComparison.Ordinal);
                ...
                if (willChangeContent || willRenameFile)
                {
                    changes.Add(new FileChange
                    {
                        ...
                        FileName = fileName,
                        NewFileName = willRenameFile ? fileName.Replace("CarToll", "DetectiveGame") : fileName,
